...
- **Live knowledge-graph panel** ✅ live — split-page chat with a Cytoscape.js canvas on the right that grows turn-by-turn. After every answer, the backend runs biomedical NER over question + answer + cited chunks, then asks the LLM (single JSON-mode call) to canonicalize the noisy NER candidate set into typed entities (with aliases) AND emit directed `(subject, predicate, object, evidence_chunk_ids)` triples. Hallucination guard: every entity must ground in NER (label or alias matches), every edge must cite a real chunk_id. Isolated nodes (entities with no relation) are dropped — a graph is connections, not vocabulary. Pan/zoom/fit controls, click-to-popover (type chip + citation pills + "Ask about this" button that prefills the chat input), in-memory per-session state with Clear/Export, persists open/closed preference in localStorage.
...
- ✅ **Live knowledge-graph panel:** split-page chat + Cytoscape.js canvas; LLM-canonicalized typed entities (with aliases) and grounded predicate edges per turn; pan/zoom/fit; click popover with citation pills + "Ask about this"; in-memory session graph
